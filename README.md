...
Проект AI-ассистента для автоматических ответов на вопросы пользователей по базе знаний компании.

Система читает `.txt`-файлы из папки `data/`, разбивает их на чанки, сохраняет в ChromaDB и формирует ответы на основе найденного контекста. Это позволяет ускорить обработку типовых запросов, снизить нагрузку на сотрудников и давать точные ответы на основе данных компании.
...
## Оценка качества ответов (RAGAS + эталонные ответы)
...
Файл `rag_evaluation.py` запускает оценку качества ответов через терминал и помогает проверять точность, релевантность и устойчивость работы ассистента. Скрипт показывает пошаговый прогресс, список файлов базы знаний, статус индекса и кэша, результаты по каждому тесту и общий итог в конце.
...
- ускорять ответы на повторяющиеся вопросы и снижать стоимость повторных запросов
