...
<p align="center">
  AI-ассистент для автоматических ответов клиентам по базе знаний
</p>
...
Прототип системы, которую можно адаптировать под техподдержку, Avito, Telegram и внутренние базы знаний.

## Бизнес-результат

- ускорение ответа клиентам
- снижение нагрузки на менеджеров
- точные ответы на основе данных компании
- снижение затрат на повторные запросы за счёт кеширования
...
python app.py --dialog-id avito_chat_42  # запуск с сохранением истории конкретного диалога
...
Для очистки памяти текущего диалога в консоли используйте команду `reset`.
...
- `HISTORY_TURNS = 3` - сколько последних пар сообщений хранится в памяти текущего диалога.
- `WARM_LEAD_HISTORY_WINDOW = 4` - сколько последних сообщений клиента учитывается при определении "тёплого" лида.
- Параметр `TOP_K` определяет, сколько чанков извлекается из базы знаний перед генерацией ответа.

История диалога теперь сохраняется по `dialog_id`, поэтому бот может помнить контекст между запусками, если использовать один и тот же идентификатор диалога.
...
- `--dataset main|warm-lead|retrieval-debug` - выбрать набор тестов
...
python rag_evaluation.py --dataset warm-lead # отдельная оценка warm lead сценариев
